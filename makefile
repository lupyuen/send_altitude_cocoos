...
LINKER_FLAGS = -mcpu=cortex-m4 -mthumb -mlittle-endian -O2 -fmessage-length=0 -fsigned-char -ffunction-sections -fdata-sections  -g3 -ggdb -T "./stm32f4/STM32F417IG_FLASH.ld" -Xlinker --gc-sections -Wl,-Map,"$(output)/app.map"  -o "$(output)/app.elf"
...
source = . ./stm32f4/src ./radios ./sensors ../cocoOS/src ../stmlib/stm32f4/src
output = ./output
modpath = ..
...
./output/%.o : ../cocoOS/src/%.c
	$(COMPILE_C) $(includes) -c $^ -o $@

./output/%.o : ../stmlib/stm32f4/src/%.c
	$(COMPILE_C) $(includes) -c $^ -o $@
